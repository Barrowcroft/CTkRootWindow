...
class CTkFrameLayout13(CTkFrameLayout):
    """CTkFrameLayout13
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._top.grid(  # type: ignore - call to CustomTknter
            row=0, column=0, columnspan=3, padx=0, pady=(0, gutter), sticky="nsew"
        )

        self._left = CTkFormattedFrame(self, width=200)
        self._left.grid(row=1, column=0, padx=(0, gutter), pady=0, sticky="nsew")  # type: ignore - call to CustomTknter

        self._main = CTkFormattedFrame(self)
        self._main.grid(row=1, column=1, padx=(0, gutter), pady=0, sticky="nsew")  # type: ignore - call to CustomTknter

        self._right = CTkFormattedFrame(self, width=200)
        self._right.grid(row=1, column=2, padx=0, pady=0, sticky="nsew")  # type: ignore - call to CustomTknter

        #  Set the frame layout's descriptors

        self._str = "CTkFrameLayout13"
        self._repr = (
            "CTkFrameLayout13 (master={master}, *args={*args}, **kwargs={**kwargs})"
        )
